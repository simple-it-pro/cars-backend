...
export class MessagesAttachmentService {
    ...
    async processVideoMessage(file: Express.Multer.File): Promise<string> {
        if (!file) {
            throw new BadRequestException(ERROR_MESSAGES.MESSAGE.FILE_REQUIRED);
        }
        ...
        if (!file.mimetype.startsWith('video/')) {
            throw new BadRequestException(
                ERROR_MESSAGES.MESSAGE.VIDEO_REQUIRED,
            );
        }
    ...
    async addSignedUrlsToMessage(message: Message): Promise<Message> {
        ...
        if (message.videoUrl) {
            message.videoUrl = await this.storageService.getFileUrl(
                message.videoUrl,
            );
        }
        ...
        if (message.forwardedFrom?.videoUrl) {
            message.forwardedFrom.videoUrl =
                await this.storageService.getFileUrl(
                    message.forwardedFrom.videoUrl,
                );
        }
        ...
        if (message.repliedMessage?.videoUrl) {
            message.repliedMessage.videoUrl =
                await this.storageService.getFileUrl(
                    message.repliedMessage.videoUrl,
                );
        }
    ...
    async deleteMessageAttachments(message: Message): Promise<string[]> {
        ...
        if (message.videoUrl) {
            try {
                await this.storageService.deleteFile(message.videoUrl);
            } catch (error) {
                const errorMsg = `Failed to delete video message: ${message.videoUrl}`;
                deletionErrors.push(errorMsg);
                console.error(errorMsg, error);
            }
        }
