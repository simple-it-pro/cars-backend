...
export class ChatsController {
    ...
    @ApiOperation({ summary: 'Отправить видео сообщение' })
    @ApiConsumes('multipart/form-data')
    @ApiBody(MESSAGE_BODIES.SEND_VIDEO_MESSAGE)
    @ApiResponse({
        status: 201,
        type: Message,
        description: 'Видео сообщение отправлено',
    })
    @ApiResponse({
        status: 400,
        description: 'Файл не предоставлен или имеет недопустимый формат',
    ...
    async sendVideoMessage(
        ...
        return this.messagesService.sendVideoMessage(chatId, sender, dto, file);
    }
