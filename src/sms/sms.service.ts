...
export class SmsService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.testMode = smsConfig.testMode;
  }
  ...
  async sendVerificationCode(phone: string, code: string): Promise<boolean> {
    ...
    try {
      ...
      this.logger.debug(`SMS.ru response: ${JSON.stringify(response.data)}`);
      ...
      if (response.data.status === 'OK') {
        ...
        if (smsStatus === 'OK') {
          ...
          this.logger.error(`SMS sending failed for ${phone}: ${smsStatus}`);
          return false;
        }
      } else {
        this.logger.error(
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          this.logger.error('No response received from SMS.ru', error.message);
        } else {
          this.logger.error('Error setting up SMS request', error.message);
        }
      } else {
